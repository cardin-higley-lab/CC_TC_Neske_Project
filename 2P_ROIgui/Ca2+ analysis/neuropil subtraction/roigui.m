function [ROI_list]=roigui(im,ROI_list,pixel_dimension)
%   
...
d.DispRange=[prctile(d.data(:),5),prctile(d.data(:),99.8)];
d.colormap=1;
dispstruct=d;
...
d.DispRange=[prctile(firstim(:),1),prctile(firstim(:),98)];
d.colormap=1;
dispstruct(end+1)=d;
...
%     l3=handle.listener(hobjs.SlMin,'ActionEvent',@SlMinCallback);
    l1=addlistener(hobjs.SlFrame,'ContinuousValueChange',@SlFrameCallback);
    l2=addlistener(hobjs.SlMax,'ContinuousValueChange',@SlMaxCallback);
    l3=addlistener(hobjs.SlMin,'ContinuousValueChange',@SlMinCallback);
end
%lh = addlistener(Hsource,property,EventName,callback) or lh = addlistener(Hsource,EventName,callback)
